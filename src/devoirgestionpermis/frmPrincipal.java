...
public class frmPrincipal extends javax.swing.JFrame {
    ...
    private void formWindowOpened(java.awt.event.WindowEvent evt) {//GEN-FIRST:event_formWindowOpened
        ...
        gst.AjouterUnEleve(eleve1);gst.AjouterUnEleve(eleve5);gst.AjouterUnEleve(eleve4);
        gst.AjouterUnEleve(eleve2);gst.AjouterUnEleve(eleve3);
        
        // Ajouter les tests au gestionnaire
        gst.AjouterUnTest(test5);gst.AjouterUnTest(test4);gst.AjouterUnTest(test3);
        gst.AjouterUnTest(test2);gst.AjouterUnTest(test1);
    ...
    private void btnInscriptionActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnInscriptionActionPerformed
     ...
     boolean trouverTest = false;
        // si rien n'est selectionner dans la liste
 ...
 // si rien n'est selectionner dans la liste
    ...
    // aucun inscrit au demarage de l'application
        ...
        {
            //si le salrie est deja inscrit 
            int i = 0;
            for(Eleve e : gst.getLesElevesInscrits())
            {
                if(gst.ExisterEleve(e))
                     {
                 trouveEleve = true;
                 eleve = e;
               //  JOptionPane.showMessageDialog(this,"cet eleve est deja inscrit");
                
               for(Test t : gst.getTousLesTests())
                            {
                                if(eleve.VerifierSiTestTermine(t))
                                {
                                    trouverTest = true;
                                    break;
                                }
            ...
            if (trouverTest)
            {
                JOptionPane.showMessageDialog(this, "Déjà inscrit");
            }
            else 
            {
                if(!trouveEleve){
    ...
    private void btnModificationActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnModificationActionPerformed
     ...
     Eleve e  =  gst.getLesElevesInscrits().get(cboElevesInscrits.getSelectedIndex());
     e.getSesTests().setTermine(Boolean.valueOf(tblTests.getValueAt(i, 3).toString()));
     }
